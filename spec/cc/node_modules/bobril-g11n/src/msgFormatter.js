...
const localeDataStorage = require('./localeDataStorage');
    ...
    switch (type) {
        case 'number': {
            if (style === 'custom' && 'format' in options) {
                return (val, opt) => { numeral.language(language); return numeral(val).format(opt.format); };
            }
            if (style === 'default') {
        ...
        case 'time': {
            if (style === 'relative') {
                ...
                if (options['noago'] === null) {
                    return (val, opt) => { return moment(val).locale(language).fromNow(opt['noago']); };
                }
            ...
            if (style === 'custom' && 'format' in options) {
                return (val, opt) => { return moment(val).locale(language).format(opt.format); };
            }
...
function compile(locale, msgAst) {
    ...
    switch (msgAst.type) {
        case 'arg':
            return ((name) => (params) => params[name])(msgAst.id);
        case 'hash':
...
//# sourceMappingURL=msgFormatter.js.map